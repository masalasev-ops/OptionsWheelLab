...
two values would state one fact twice. `closed_by_choice` is written from 4.4 by
the close a maker chooses [D-W54] and was in the `CHECK` from migration 8, before
anything wrote it. It is recovered by asking whether a bound had
